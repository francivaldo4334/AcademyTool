...
fun ColumnScope.ClientsTable() {
  ...
  Table(
          headerItems =
                  listOf(
                          ...
                          HeaderItem(title = R.strings.name, onSortAction = {}),
                          HeaderItem(
                          ...
                          HeaderItem(title = R.strings.phone_or_whatsapp, minWidth = 200.dp),
                  )
  ) {
    ...
    row(
            key = 0,
            {
              ProfileImage(fileName = "teste.jpeg", modifier = Modifier.padding(12.dp).size(32.dp))
            },
  ...
  if (openDetails) {
    StudentDetails(onDismissRequest = { openDetails = false })
  }
}
