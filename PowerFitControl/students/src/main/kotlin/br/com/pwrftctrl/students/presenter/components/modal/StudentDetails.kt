...
@Composable
private fun RowBottomButtons(formStudent: Form) {
    Row(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(16.dp)) {
            if (formStudent.isOnlyRead.value) {
                TextButton(
                        onClick = { formStudent.setIsOnlyRead(false) },
                ) { Text(R.strings.edit) }
            }
            RedButton(
                    onClick = {
            ...
            ) { Text(if (formStudent.isOnlyRead.value) R.strings.exclude else R.strings.cancell) }
        }
        if (!formStudent.isOnlyRead.value) {
            TextButton(
                    onClick = { formStudent.onSubmit() },
            ) { Text(R.strings.save) }
        }
    }
}
...
fun StudentDetails(
    ...
    LaunchedEffect(Unit) { formStudent.setIsOnlyRead(true) }
    val STUDENT = 0
    val ADDRESS = 1
    val REGISTER = 2
    var optionSelected by remember { mutableStateOf(STUDENT) }
    BaseModal(
            autoDimiss = true,
                ...
                Column(modifier = Modifier.height(640.dp)) {
                    ...
                    FormSelection(
                            options =
                                    listOf(
                                            FormItemSelection(
                                                    { Text(R.strings.student) },
                                                    { optionSelected = STUDENT }
                                            ),
                                            FormItemSelection(
                                                    { Text(R.strings.address) },
                                                    { optionSelected = ADDRESS }
                                            ),
                                            FormItemSelection(
                                                    { Text(R.strings.student_register) },
                        ...
                        LazyColumn {
                            when (optionSelected) {
                                ...
                                REGISTER ->
                                        FieldsStudentRegistrationData(formStudentRegistrationData)
                            }
                        }
                    }
