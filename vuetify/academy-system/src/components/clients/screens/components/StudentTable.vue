  ...
  <v-data-table-virtual :items="getClients(props.clients)" :headers="headers.filter(header => !header.hidden)"
    ...
    <template v-slot:[`item.chips`]="{ value }">
      <v-row class="ga-1 justify-center">
        <v-chip v-for="chip in value" :key="chip" :class="chip.count === 0 && 'pa-1'" :color="getChipColor(chip.type)"
          :prepend-icon="chip.count > 0 && getChipIcon(chip.type)" density="compact" label>
    ...
    <template v-slot:[`item.contact`]="{ value }">
      <v-row class="d-flex ga-1 justify-end px-0">
        <v-chip @click="copy(contact.content); snackbar = true;" v-for="contact in value" :key="contact" color="green"
          :prepend-icon="Copy">
          {{ contact.content.replace(/(\d{2})(\d{1})(\d{4})(\d{4})/, '($1) $2 $3-$4') }}
          <template v-slot:append>
            <v-btn icon variant="text" class="v-icon--end" size="24">
              <v-icon :icon="contact.type === 'whatsapp' ? MessageCircle : Phone" :size="16"
                @click.stop="redirect(contact.content)" />
